...
localparam integer LP_AXI_BURST_LEN      = 1024/LP_DW_BYTES < 256 ? 1024/LP_DW_BYTES : 256;
...
// localparam integer G_WORD_WIDTH = 32;
// localparam integer SCORE_WORD_WIDTH = 128;
...
assign ap_start_pulse = (ap_start & ~ap_start_r);// | (state[1] & ~state_r[1]) | (state[0] & ~state_r[0]);
...
assign state_change_pulse = ((state[1] & ~state_r[1]) | (state[0] & ~state_r[0]));

logic [LP_NUM_READ_CHANNELS-1:0][C_M_AXI_GMEM_ADDR_WIDTH-1:0] addr_offset;
logic [31:0] size;
assign addr_offset = ((state[0]==0) ? a : a+64'd256); // TODO: change to 256
assign size = ((state[0]==0) ? 32'd1 : 32'd1);
...
inst_axi_read_master ( 
  ...
  .ctrl_start     ( ap_start_pulse | (state_change_pulse & state[1] & ap_start_r)   ) ,
  .ctrl_done      ( read_done              ) ,
  .ctrl_offset    ( addr_offset            ) ,
  .ctrl_length    ( size               ) ,
...
// Combinatorial Adder
 krnl_vadd_rtl_adder #( 
   .C_DATA_WIDTH       ( C_M_AXI_GMEM_DATA_WIDTH ) ,
  //  .C_OUT_DATA_WIDTH   ( SCORE_WORD_WIDTH ),
   .C_NUM_CHANNELS ( LP_NUM_READ_CHANNELS    ) 
 )
 inst_adder ( 
   .clk     ( ap_clk            ) ,
